...
清洗返程已改为：沿杯口轴退出 → 保持倒料姿态垂直抬到 `z=0.45 m` → 高位回正
→ 直接跨区返回出料口，删除低位回倒料点的绕行。倒料主翻腕提高到 70%/50%，
摇料为 60%/45%。本次只完成软件编译、约束检查和 dry-run；按操作员要求尚未执行
机械臂。下一次真机必须先分段确认高位回正和跨区净空，再跑完整循环。
