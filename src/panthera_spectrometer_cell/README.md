...
| `/spectrometer_cell/restart_gripper` | 仅在无活动动作、未持杯且光谱仪工位无杯时恢复夹爪驱动；随后用现有夹爪控制器执行 50→45→50 mm（原本接近全开）或当前位置→50 mm（原本未全开）的可动性自检，不移动机械臂。 |

生产夹爪预检只要求请求之后收到一帧新的电机 7 反馈且 `fault=0`。硬件层在反馈超时
时最多执行一次夹爪复位并等待复位后的新帧；不使用位置、速度、力或进度门限。
人工恢复的动作自检独立于生产预检：编码器沿命令方向发生任意新变化即确认夹爪可动，
不设置移动量、位置误差、速度或力门限；最终持续保持全开目标。失败只返回给 HMI，
不把状态机送入 `ERROR`。
