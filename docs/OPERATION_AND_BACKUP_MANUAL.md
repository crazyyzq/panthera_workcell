...
| `outlet_1_return` | `[0.484046, -0.097, 0.215]`（放回比取杯高 3 mm，自动跟随） |
| `outlet_2_grasp` | `[0.484102, 0.085769, 0.211]` |
| `outlet_2_return` | `[0.484102, 0.085769, 0.214]`（放回比取杯高 3 mm，自动跟随） |
...
夹爪当前开/合轨迹时长分别为 `2.0513 s` / `1.4359 s`，在上一版 `2.6667 s` / `1.8667 s`
基础上再提速 30%；夹杯闭合目标仍为 `0.015 m`，持续保持逻辑不变。编码器确认窗口和命令
