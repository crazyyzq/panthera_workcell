...
3. 可使用 0.5–20 mm 平移步进或 0.1–10° 旋转步进按钮；也可先填入当前实测值，直接修改基座坐标 XYZ/RPY 后执行。每次输入目标与当前位置的空间距离仍不得超过 20 mm、姿态变化不得超过 10°。
4. 按需测试夹爪和毛刷；毛刷速度可保存为下次默认值。
5. 页面显示目标、实测、位置/姿态误差以及有界修正次数。确认合适后点击 `保存点位并热重载`；后端保存最终命令目标，更新关联接近点，编译全部路线后原子保存。
