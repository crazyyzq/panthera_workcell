...
工控机网络地址会随现场网络变化，不在工程内写死。远程维护使用已配置的 SSH
跳板/反向隧道；需要远程查看页面时转发工控机的 `127.0.0.1:8080`。
...
- 位置速度硬件模式（`position_velocity`，生产默认）
- 固定轨迹 Motion Server（生产运行不实时规划）
- C++ 光谱检测长期状态机
- 激光传感器适配器（启动时未连接则使用配置中的固定参考位置）
- Gemini305 相机自动恢复 watchdog（按需启动相机；未连接不阻塞工作站 READY）
...
编码器位于全零 Home、HMI 服务全部就绪后才报告成功；默认工作速度为 100%。
...
该脚本只在流程已结束、杯子已释放、机械臂静止且编码器确认位于全零 Home 时关闭：
...
- 夹爪恢复：仅在无活动动作、未持杯且光谱仪工位无杯时开放；点击后只恢复夹爪驱动，不重启机械臂。
- 关节状态：`fault=0` 显示正常，非零显示厂商故障含义；控制模式显示来自本次工作站启动配置，不使用厂商状态帧类型字节推断。
...
| `spectrometer_pick` | `[0.162, 0.482, 0.317]` |
...
当前开爪/合爪轨迹时长分别为 `1.3675 s` / `0.9573 s`，硬件速度上限保持
`0.01875 m/s`；持续保持逻辑不变。命令收尾余量为 `5 s`，避免夹爪驱动短暂保护时
立即重复冲击。

每轮生产任务开始前只检查夹爪电机 7 是否返回一帧新的、`fault=0` 的反馈，不使用
位置、速度、力或到位进度门限。若 3 秒内没有新反馈，硬件层只执行一次夹爪复位并
等待复位后的新反馈；暂时失败时状态机留在 `SELECT_TASK` 重试，不把整条产线锁进
`ERROR`。HMI 的“恢复夹爪”按钮调用相同恢复入口。
...
3. 在工控机打开 HMI：`http://127.0.0.1:8080`。
